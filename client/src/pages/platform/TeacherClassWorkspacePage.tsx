...
import { ArrowLeft, CalendarDays, CheckCircle2, FileText, Users } from "lucide-react";
...
import type { AttendanceStatus, EntityStatus, StudentStatus } from "@/lib/domain/types";

type TeacherClassWorkspaceView = "sessions" | "attendance" | "students" | "materials";
...
const viewMeta: Record<TeacherClassWorkspaceView, { title: string; description: string; icon: typeof CalendarDays }> = {
  sessions: {
    title: "Sessions",
...
export default function TeacherClassWorkspacePage({ classId, view }: TeacherClassWorkspacePageProps) {
  ...
  const resources = state.resources.filter(item => lessonIds.has(item.lessonId));
  const attendanceRecords = state.attendance.filter(item => item.classGroupId === classGroup?.id);
  const activeSession = sessions.find(item => item.id === selectedSessionId) ?? sessions[0];
  ...
  useEffect(() => {
    ...
    const sessionKeys = new Set([activeSession.id, activeSession.eventId].filter(Boolean));
    ...
    classGroup.studentIds.forEach(studentId => {
      const record = state.attendance.find(
        item => item.classGroupId === classGroup.id && item.studentId === studentId && sessionKeys.has(item.sessionId)
      );
  ...
  const nav = (
    <div className="portal-simple-tabs teacher-class-tabs" aria-label="Class sections">
  ...
  const saveAttendance = async () => {
    if (!classGroup || !activeSession) return;
    const statuses = classGroup.studentIds.reduce<Record<string, AttendanceStatus>>((acc, studentId) => {
  ...
  function renderMain() {
    if (view === "sessions") {
      return (
        <DataTableCard title="Class sessions" subtitle={`${sessions.length} sessions`}>
              ...
              {sessions.length ? (
                ...
                <tr>
                  <td colSpan={5}>
                    ...
                    <small>Class sessions will appear here when scheduled.</small>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="portal-simple-stack">
          <section className="platform-workflow-card teacher-class-attendance-panel">
            <div className="platform-card-title compact">
              <div>
                <span>{activeSession ? formatDateTime(activeSession.startsAt) : "No session"}</span>
            ...
            <div className="platform-attendance-control-grid">
              ...
              <label>
                Session
                <select value={activeSession?.id ?? ""} onChange={event => setSelectedSessionId(event.target.value)}>
                  {sessions.map(session => (
                ...
                return (
                  <article key={studentId}>
                    <div>
                      ...
                      <span>{student?.currentLevel ?? course?.title ?? "Learner"}</span>
                    </div>
                    <div>
            ...
            <button type="button" onClick={() => void saveAttendance()} disabled={attendanceSaving || !activeSession}>
              {attendanceSaving ? "Saving..." : "Save attendance"}
            </button>
          </section>

          <DataTableCard title="Attendance records" subtitle={`${attendanceRecords.length} records`}>
            <table className="teacher-class-workspace-table">
              <thead>
                <tr>
                  <th>Student</th>
                ...
                {attendanceRecords.length ? (
                  attendanceRecords.map(record => {
                    ...
                    return (
                      <tr key={record.id}>
                        <td>
                          <strong>{user?.name ?? "Student"}</strong>
                          <small>{student?.currentLevel ?? course?.title ?? "Learner"}</small>
                        </td>
                        <td>{session?.title ?? "Session"}</td>
                        <td>
                          <StatusBadge tone={attendanceTone(record.status)}>{record.status}</StatusBadge>
                        </td>
                  ...
                  <tr>
                    <td colSpan={4}>
                      ...
                      <small>Attendance records will appear after a class session is saved.</small>
                    </td>
                  </tr>
                )}
              </tbody>
    ...
    if (view === "students") {
      return (
        <DataTableCard title="Class students" subtitle={`${enrollments.length || currentClass.studentIds.length} learners`}>
          <table className="teacher-class-workspace-table">
            <thead>
              <tr>
                <th>Name</th>
            ...
            <tbody>
              {(enrollments.length ? enrollments : currentClass.studentIds.map(studentId => ({ id: studentId, studentId }))).map(item => {
                const student = state.students.find(profile => profile.id === item.studentId);
                const user = state.users.find(profile => profile.id === student?.userId);
                const attendanceRate = "attendanceRate" in item ? item.attendanceRate : undefined;
                return (
                  <tr key={item.id}>
                    <td>
                      <strong>{user?.name ?? "Student"}</strong>
                    ...
                    <td>{student?.currentLevel ?? course?.title ?? "Course learner"}</td>
                    <td>
                      <StatusBadge tone={studentTone(student?.status ?? "active")}>{student?.status ?? "active"}</StatusBadge>
                    </td>
                    <td>{typeof attendanceRate === "number" ? `${attendanceRate}%` : "Not recorded"}</td>
                  </tr>
                );
              })}
            </tbody>
    ...
    return (
      <DataTableCard title="Class materials" subtitle={`${resources.length} resources`}>
        <div className="teacher-material-list teacher-class-material-list">
          {resources.length ? (
            resources.map(resource => {
              const lesson = lessons.find(item => item.id === resource.lessonId);
              return (
                <article key={resource.id}>
                  ...
                  >
                    {materialSavingKey === resource.id ? "Saving..." : resource.published ? "Published" : "Publish"}
                  </button>
                </article>
              );
            })
            ...
            <article>
              <div>
                ...
                <small>Published resources for this class will appear here.</small>
              </div>
            </article>
          )}
        </div>
  ...
  return (
    <PlatformShell role="teacher" title={`${currentClass.name} ${meta.title}`}>
      <WorkspaceLayout
        ...
        actions={
          <Link className="platform-secondary-button" href={`/app/teacher/classes/${currentClass.id}`}>
            <ArrowLeft size={15} />
            Class overview
          </Link>
        }
